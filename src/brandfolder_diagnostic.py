...
def diagnose_sku(sku: str) -> None:
    ...
    print("=" * 70)
    print("BRANDFOLDER API DIAGNOSTIKA")
    print("=" * 70)
    print(f"Meklētais SKU:               {normalized_sku}")
    print(f"API atgriezti aktīvi pirms filtra: {len(raw_data)}")
    print(f"API included ieraksti:       {len(raw_included)}")

    included_types: dict[str, int] = {}
    for item in raw_included:
        if isinstance(item, dict):
            item_type = str(item.get("type") or "(nav type)")
            included_types[item_type] = included_types.get(item_type, 0) + 1

    if included_types:
        print("Included tipi:")
        for item_type, count in sorted(included_types.items()):
            print(f"  {item_type}: {count}")

    for number, asset in enumerate(raw_data, start=1):
        if not isinstance(asset, dict):
            continue
        ...
        print("\n" + "-" * 70)
        print(f"Neapstrādāts aktīvs {number}")
        print(f"ID:              {asset.get('id', '')}")
        print(f"Type:            {asset.get('type', '')}")
        print(f"Name:            {attrs.get('name', '')}")
        print("Relationship keys: " + ", ".join(sorted(rels.keys())))
        print("Attribute keys:    " + ", ".join(sorted(attrs.keys())))

        for key in ("tags", "tag_names", "keywords"):
            if key in attrs:
                print(f"attributes.{key}: {attrs.get(key)!r}")
