...
def main() -> int:
    ...
    if args.sku:
        normalized_sku = normalize_sku(args.sku)
        product = get_product_by_sku(normalized_sku)
        ...
        selection_label = f"SKU: {normalized_sku}"
        ...
        if args.brand:
            selection_label = f"Zīmols: {args.brand}"
            print(
                f'Pēc zīmola filtra "{args.brand}" '
                f"atrasti {len(filtered_products)} produkti."
            )
        else:
            selection_label = "Visi WooCommerce produkti"

        if args.offset:
            selection_label += f", offset {args.offset}"

        if args.limit is not None:
            selection_label += f", limits {args.limit}"
    ...
    try:
        ...
        print(
            "\nHTML atskaiti neizdevās izveidot: "
            f"{error}"
        )
    else:
        print("\nHTML atskaite izveidota:")
        print(report_file)
